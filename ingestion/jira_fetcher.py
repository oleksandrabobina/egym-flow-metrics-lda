...
SHEET_JIRA_KEYS_RELEASES_GID = os.getenv("SHEET_JIRA_KEYS_RELEASES_GID")
...
def fetch_jira_project_keys_from_mapping():
    ...
    if not MAPPING_WEB_APP_URL or not SHEET_JIRA_KEYS_RELEASES_GID:
        print("Error: MAPPING_WEB_APP_URL or SHEET_JIRA_KEYS_RELEASES_GID is missing in environment variables.")
        return []

    try:
        separator = "&" if "?" in MAPPING_WEB_APP_URL else "?"
        target_url = f"{MAPPING_WEB_APP_URL}{separator}gid={SHEET_JIRA_KEYS_RELEASES_GID}"
